    ...
    <ScrollView style={styles.tableContainer} >
        {invoiceItems?.map((item, index) => (
          <View key={index} style={styles.tableRow}>
            ...
            <Text style={styles.cell}>{Currecy}:{item.price}</Text>
            <Text style={styles.cell}>{Currecy}:{item.total_Price}</Text>
          </View>
        ))}
      </ScrollView>
      ...
      <View style={styles.total}>
        <Text style={styles.totalText}>Total Amount:</Text><Text style={{textAlign:'right',marginRight:15,fontWeight:'bold'}}>{Currecy}:{InvTotalPrice}</Text>

...
// Define your SalesReportList component
const SalesReportList = ({ loading, data }) => {
        ...
        <View>
            ...
            <View>
                <Table style={styles.table}>
                    {/* Table headers */}
                    <Row data={['#', 'o_id', 'Name', 'qty', 'price', 'total', 'Date', 'More']} style={styles.head} textStyle={styles.headText} flexArr={[2, 6, 7, 4, 6, 6, 6, 4]} />
                    {/* Table rows */}
                    <ScrollView>
                        {currentItems.map((rowData, index) => (
                            <Row
                                ...
                                data={[
                                    ...
                                    <Text style={styles.PriceQty}>{rowData.order_id}</Text>,
                                    // <Text style={styles.longText}>{rowData._id.ItemId._id.categoryId.CategoryName}</Text>,
                                    <Text style={styles.longText}>{rowData.ItemId && rowData.ItemId.categoryId ? rowData.ItemId.categoryId.CategoryName : 'N/A'}</Text>,
                                    // <Text style={styles.longText}>{rowData.description}</Text>,
                                    <Text style={styles.PriceQty}>{rowData.order_qty}</Text>,
                                    <Text style={styles.PriceQty}>${rowData.price}</Text>,
                                    <Text style={styles.PriceQty}>${rowData.total_Price}</Text>,
                ...
                <View style={styles.modalContainer}>
                    <View style={styles.modalContent}>
                        ...
                        <ScrollView style={styles.modalScrollView}>
                            ...
                            <Text style={styles.modalText}><Text style={styles.modalLabel}>OrderId:</Text> {selectedItem?.order_id || 'N/A'}</Text>
                            <Text style={styles.modalText}><Text style={styles.modalLabel}>Description:</Text> {selectedItem?.description || 'N/A'}</Text>
                            <Text style={styles.modalText}><Text style={styles.modalLabel}>OrderQty:</Text> {selectedItem?.order_qty || 'N/A'}</Text>
                            <Text style={styles.modalText}><Text style={styles.modalLabel}>Sale Price:</Text> ${selectedItem?.price || 'N/A'}</Text>
                            <Text style={styles.modalText}><Text style={styles.modalLabel}>Price:</Text> ${selectedItem?.total_Price || 'N/A'}</Text>
